...
class SUNRAIL_API_Response {
  
  function setup() {
    global $sunrail_api;
    $this->include_values = array();
    $this->exclude_values = array();
    if ($sunrail_api->query->include) {
      $this->include_values = explode(',', $sunrail_api->query->include);
    }
    // Props to ikesyo for submitting a fix!
    if ($sunrail_api->query->exclude) {
      $this->exclude_values = explode(',', $sunrail_api->query->exclude);
  ...
  function get_json($data, $status = 'ok') {
    ...
    $data = apply_filters('sunrail_api_encode', $data);
    ...
    if (function_exists('json_encode')) {
      ...
      if (version_compare(PHP_VERSION, '5.3') < 0) {
        ...
        if ($sunrail_api->query->json_encode_options) {
          $json_encode_options = $sunrail_api->query->json_encode_options;
        }
    ...
    if ($sunrail_api->query->json_unescaped_unicode) {
  ...
  function respond($result, $status = 'ok') {
    ...
    if ($sunrail_api->query->dev || !empty($_REQUEST['dev'])) {
    ...
    } else if ($sunrail_api->query->redirect) {
      $url = $this->add_status_query_var($sunrail_api->query->redirect, $status);
      wp_redirect($url);
    } else if ($sunrail_api->query->callback) {
      // Run a JSONP-style callback with the result
      $this->callback($sunrail_api->query->callback, $json);
    } else {
      // Output the result
      $this->output($json);
    }
