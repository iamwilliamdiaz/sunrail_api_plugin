...
class SUNRAIL_API_Query {
  ...
  function __construct() {
    // Register SUNRAIL API query vars
  ...
  function get_legacy_controller($json) {
    ...
    if ($json == 'submit_comment') {
      if ($sunrail_api->controller_is_active('respond')) {
        return 'respond';
      } else {
        $sunrail_api->error("The 'submit_comment' method has been removed from the Core controller. To use this method you must enable the Respond controller from WP Admin > Settings > SUNRAIL API.");
      }
    } else if ($json == 'create_post') {
      if ($sunrail_api->controller_is_active('posts')) {
        return 'posts';
      } else {
        $sunrail_api->error("The 'create_post' method has been removed from the Core controller. To use this method you must enable the Posts controller from WP Admin > Settings > SUNRAIL API.");
      }
    } else {
      return 'core';
    }
  ...
  function get_method($controller) {
    ...
    if (empty($method)) {
      ...
      return false;
    } else if (method_exists("SUNRAIL_API_{$controller}_Controller", $method)) {
      ...
      if (is_search()) {
        ...
        if (empty($_GET['json'])) {
          $sunrail_api->error("Uknown method '$method'.");
        }
